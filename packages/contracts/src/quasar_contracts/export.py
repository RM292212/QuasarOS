"""
QuasarOS Deterministic JSON Schema & Consumer Type Exporter

Exports standardized JSON Schema (Draft 2020-12 compatible) files and TypeScript
declarations from Pydantic V2 models, ensuring reproducible build artifacts and zero drift.
"""
...
from quasar_contracts.errors_warnings import ScientificDiagnostic
from quasar_contracts.horizontal_grids import HorizontalGridContract, SpatialBoundingBox
...
from quasar_contracts.validation_state import ValidationCheckResult, ValidationReport
...
EXPORT_MODELS: Dict[str, Type[BaseModel]] = {
    ...
    "dataset_identity.schema.json": DatasetIdentity,
    "canonical_variable.schema.json": CanonicalVariableContract,
    "horizontal_grid.schema.json": HorizontalGridContract,
    "vertical_coordinate.schema.json": VerticalCoordinateContract,
    "time_semantics.schema.json": TimeSemanticsContract,
    "source_asset.schema.json": ImmutableSourceAsset,
    "quality_control.schema.json": QualityControlContract,
    "provenance_lineage.schema.json": LineageRecord,
    "licence_contract.schema.json": LicenceContract,
    "validation_report.schema.json": ValidationReport,
    "dataset_capabilities.schema.json": DatasetCapabilitiesContract,
...
def sort_dict(item: Any) -> Any:
    """Recursively sort dictionary keys for deterministic JSON generation."""
...
def generate_json_schema(model_cls: Type[BaseModel]) -> Dict[str, Any]:
    """Generate deterministic Draft 2020-12 compatible JSON schema from a Pydantic model."""
    raw_schema = model_cls.model_json_schema()
    raw_schema["$schema"] = "https://json-schema.org/draft/2020-12/schema"
    return sort_dict(raw_schema)


def export_schemas_to_directory(output_dir: Path) -> List[Path]:
    """Export all registered model schemas to a target directory."""
    output_dir.mkdir(parents=True, exist_ok=True)
    generated_files = []

    for filename, model_cls in EXPORT_MODELS.items():
        schema_dict = generate_json_schema(model_cls)
...
def verify_schemas_in_directory(target_dir: Path) -> Tuple[bool, List[str]]:
    """
    Verify whether disk schema files match currently defined Pydantic models.
    Returns (True, []) if in sync, or (False, [drift_messages]) if drift detected.
    """
    drift_messages = []
    ...
    for filename, model_cls in EXPORT_MODELS.items():
        ...
        if not file_path.exists():
            drift_messages.append(f"Missing schema file on disk: {file_path}")
            continue
        ...
        with open(file_path, "r", encoding="utf-8") as f:
            try:
                ...
                drift_messages.append(f"Failed to parse JSON from {file_path}: {e}")
                continue

        current_schema = generate_json_schema(model_cls)
        disk_sorted = sort_dict(disk_schema)
        current_sorted = sort_dict(current_schema)

        if disk_sorted != current_sorted:
            drift_messages.append(f"Schema drift detected in {filename}")

    return (len(drift_messages) == 0, drift_messages)


def generate_typescript_declarations(output_file: Path) -> Path:
    """Generate authoritative TypeScript declaration file for front-end consumers."""
