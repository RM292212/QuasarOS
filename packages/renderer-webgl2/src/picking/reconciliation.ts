/**
 * @quasar/renderer-webgl2 Picking Subsystem - Authoritative Reconciliation Client Integration
 *
 * Implements reconcilePickWithBackend(pickInput, clientOrQueryClient, options):
 * 1. Takes provisional pick result from WebGL2VolumePicker / ProvisionalPickMapper.
 * 2. Dispatches POST /api/v1/queries/reconcile-pick to TASK-05 FastAPI service via @quasar/client.
 * 3. Returns unified reconciliation result displaying both provisional WebGL2 render value and authoritative native NetCDF value.
 *
...
import type { WebGL2PickResult } from './webgl2_volume_picker.ts';

export interface UnifiedPickReconciliationResult {
  /** Approximate provisional WebGL2 raymarching feedback */
  provisional: {
...
/**
 * Reconcile a WebGL2 volume pick with the authoritative backend query service.
 *
 * @param pickInput WebGL2PickResult or ProvisionalPickResult or ReconcilePickRequest
...
export async function reconcilePickWithBackend(
  pickInput: WebGL2PickResult | ProvisionalPickResult | ReconcilePickRequest,
  ...
  if ('reconcileRequest' in pickInput) {
    ...
    // WebGL2PickResult with successful hit
