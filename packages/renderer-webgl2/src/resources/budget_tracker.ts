/**
 * @quasar/renderer-webgl2 GPU Memory Budget Tracker
 * Tracks active texture and uniform buffer allocations against strict limits (default 50 MiB texture ceiling).
 */

import { WebGL2MemoryBudgetExceededError } from '../errors.ts';
import type { WebGL2BudgetTrackerOptions, WebGL2MemoryStats } from '../types.ts';

export class WebGL2BudgetTracker {
  ...
  constructor(options: WebGL2BudgetTrackerOptions = {}) {
  ...
  recordTextureAllocated(sizeInBytes: number): void {
    if (this._textureMemoryBytes + sizeInBytes > this.maxTextureBudgetBytes) {
      throw new WebGL2MemoryBudgetExceededError(
